...
* Bugs
** TODO [#D] State of keyword tree is not restored when refreshing
** TODO [#E] Tab should choose selection in typeahead not go to next suggestion.
** TODO [#F] project tree not expanding on startup (pics displayed ok tho)
...
** TODO [#C] return doesn't close popover
