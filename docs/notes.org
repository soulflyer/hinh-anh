...
May not be worth doing. Maybe reset filter-stars if picture-list is empty? Or alter filter to include pictures tagged with red.
** TODO [#C] Search tree for a project or keyword.
*** TODO [#C] Call API for a full list of keywords (or maybe extract from keyword tree)
*** TODO [#C] re-com typeahead
*** TODO [#D] Modify or bypass popover-wrapper to fit typeahead
*** TODO [#D] On found, call API to get the parents of the keyword
(or add to API, get path from keyword)
*** TODO [#D] Pass path to :expand-path
** TODO [#C] keyword set needs an update-from-current button
