...
** TODO [#B] Error message is not visible.
** TODO [#C] U key only works when keywording has focus.
** TODO [#A] Possible to add multiple identical keywords to keywording set.
* Improvements
** Filters
...
** TODO [#A] Consider refreshing pictures
Might be a better way to go than updating them locally as well as via the api.
