...
* Bugs
** TODO [#B] Refresh after adding a keyword gets a project even when looking at a keyword.
...
** TODO [#F] Correct info display when pictures are selected by keyword
...
*** TODO [#B] Store best image in :sample if it is empty
*** TODO editing functions
**** DONE Make del button raise a popover for confirmation
     CLOSED: [2018-08-08 Wed 19:22]
**** TODO [#C] Make esc cancel popovers
**** TODO [#C] Make return trigger highlighted popover.
**** TODO [#C] Highlight edit field when popover opens
**** DONE add-keyword
...
**** TODO [#B] Button to set sample
*** TODO [#D] Handle orphaned and empty keywords
*** TODO [#E] Change keyword buttons to modals?
*** TODO [#I] sample stores a path with .jpg on the end. Remove it.
Removed by the API. Should be removed from image-lib, but functions ok with or without.
*** TODO [#G] Make keyword browsing default to best image.
** DONE Add select all
...
** TODO [#D] Add ability to jump to project from a picture
