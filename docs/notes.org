...
* Bugs
** TODO [#A] Keyword shortcuts are out of order when adding one rearranges the order.
Corrects itself when tabbing away and switching the focus to and from the pictures panel.
** TODO [#B] Can't add keywording button for new keyword if there are possible completions.
...
** TODO [#F] Tab should choose selection in typeahead not go to next suggestion.
...
** TODO [#B] Switch adding keywords to popovers
...
** TODO [#F] Add keys to go directly between left-panel screens
